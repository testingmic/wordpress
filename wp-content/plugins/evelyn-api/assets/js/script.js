$(function() {
	...
	$(`div[class~="syncronizeModal"] button[class~="confirm-sync"]`).on('click', function(evt) {
		$(`div[class~="syncronizeModal"] button`).prop('disabled', true);
		$(`div[class~="syncronizeModal"] button[class~="confirm-sync"]`).html(`Processing request...`);

		var formData = {
			'action': 'syncronizeData'
		}

		$.post(ajaxurl, formData, function(resp) {
			$(`div[class~="syncronizeModal"] button`).prop('disabled', false);
			$(`div[class~="syncronizeModal"] button[class~="confirm-sync"]`).html(`Confirm`);

			if(resp.status == 'success') {
				alert(resp.result);
				window.location.href = $(`input[name="admin_pg_url"]`).val() + "admin.php?page=evelynpos_api";
			}
		}, 'json').catch((err) => {
			$(`div[class~="syncronizeModal"] button`).prop('disabled', false);
			$(`div[class~="syncronizeModal"] button[class~="confirm-sync"]`).html(`Confirm`);
